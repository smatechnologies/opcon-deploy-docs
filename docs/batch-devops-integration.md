...
description: "Use the Devops.SMAOpConDeployClient.exe to export schedule and script definitions from OpCon and deploy them through DevOps release pipelines or directly from a repository branch."
tags:
  - Reference
  - Automation Engineer
...
When deploying a script version, the script information must exist in the Deploy repository.
...
|---|---|---|
| REST API authentication failure when using REPO action | The DevOps project name (`-pj`), repository name (`-r`), or credentials supplied in the config.ini file are invalid or do not have access to the repository | Verify the project and repository names are correct and that the credentials configured for the DevOps REST API in config.ini are valid |
| EXPORT fails because the schedule or script does not exist in the Deploy repository | The schedule or script named in `-tn` was not found in the Deploy repository, preventing the export from completing | Confirm the schedule or script name is correct and that it has been previously imported into the Deploy repository |
| REPO action fails because the specified branch does not exist | The branch supplied via `-b` cannot be found in the DevOps repository | Check that the branch name is spelled correctly and that it exists in the target DevOps repository before running the REPO action |

## Key terms

**EXPORT action** — the `Devops.SMAOpConDeployClient.exe` action that extracts a schedule or script version from a source OpCon system, saves it into the Deploy repository, and writes it to a file in the specified directory so it can be committed to a DevOps repository branch.

**DEPLOY action** — the `Devops.SMAOpConDeployClient.exe` action that reads a schedule or script artifact file produced during a release pipeline build, saves it as a new version in the Deploy repository, and then deploys it to the specified target OpCon system; an optional release tag can be recorded in the deployment record and schedule documentation.

**REPO action** — the `Devops.SMAOpConDeployClient.exe` action that retrieves a schedule or script definition directly from a specified branch and path in a DevOps repository using the DevOps REST API, saves it as a new version in the Deploy repository, and deploys it to the target OpCon system.
