---
title: Using OpCon Deploy
description: "Understand how OpCon Deploy extracts, stores, and deploys schedule definitions between OpCon environments using a versioned central repository."
tags:
  - Conceptual
  - Automation Engineer
  - Getting Started
...
OpCon Deploy moves schedule definitions between OpCon environments by extracting them from a source system, storing them as versioned JSON definitions in a central repository, and deploying them to target systems with optional transformation rules applied. Every import creates a new version, and no existing definition is ever overwritten. This page explains the structure of a schedule definition, the JSON format used for storage, and the two-phase deployment process that ensures controlled, auditable promotion from development to production.

## Schedule definition contents
...
* All Annual Plan calendars discovered during the extraction process associated with frequencies. Only future dates are added to the extraction
...
* If the OpCon server type is a Non-production server, the process stops and an error message is returned. You then have the option to cancel the deployment or override the error condition. If override is selected, go to the OpCon Deploy Step
...
**Central repository** — the versioned database at the heart of OpCon Deploy where every imported schedule, package, transformation rule, and script is stored as a numbered JSON definition; no existing definition is ever overwritten, so every version remains available for deployment or rollback.

**Schedule definition** — the complete JSON extract of a schedule from an OpCon environment, encompassing job definitions, global properties, resources, thresholds, calendars, internal and external dependencies, and any embedded script information captured at import time.

**Transformation rule** — a set of value-substitution instructions applied to a schedule definition during deployment to adapt it for a target environment, such as remapping machine names, user codes, or global property values without modifying the stored repository definition.

**Deployment record** — the record created in the OpCon Deploy database each time a schedule or package is deployed, capturing the deployed definition, the definition previously on the target system, a timestamp, the deploying user, and an MD5 hash used to detect out-of-band changes on the target.
